...
from projutil import currentProjectDirectory, currentProjectFilePath, currentScenesDirectory, iterSceneNames, SCENE_EXT, templatePathFromScenePath, gSettings, setCurrentProjectFilePath
...
def main() -> None:
    if len(sys.argv) > 1:
        # Replace disk settings by in-memory settings to avoid persistent changes.
        global gSettings
        transientSettings = QSettings(QSettings.IniFormat, QSettings.UserScope, "", "")
        for k in gSettings.allKeys():
            transientSettings.setValue(k, gSettings.value(k))
        del gSettings
        gSettings = transientSettings
        setCurrentProjectFilePath(sys.argv[1])
    ...
    outputPath = FilePath(sys.argv[2]).join('content', '__autogenerated', 'generated.hpp') if len(sys.argv) > 2 else FilePath(__file__).abs().parent().parent().join('MelonPan', 'content', '__autogenerated', 'generated.hpp')
